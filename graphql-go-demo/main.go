...
func main() {
	...
	data, err := executeListQuery(schema)
	if err != nil {
		log.Fatalf("failed to execute list query, error: %v", err)
	}
	fmt.Printf("%s\n", data)
	data, err = executeGetQuery(schema)
	if err != nil {
		log.Fatalf("failed to execute get query, error: %v", err)
	}
	fmt.Printf("%s\n", data)

}

func executeListQuery(schema graphql.Schema) ([]byte, error) {
	// Query
	query := `
		{
			list {
...
func executeGetQuery(schema graphql.Schema) ([]byte, error) {
	// Query
	query := `
		{
			tutorial(id: 1) {
